...
SYSTEM_THROUGHPUT = Counter(
    "mpr_throughput_total", "Total transaction velocity tracks", ["status"]
)
LATENCY_PROFILE = Histogram(
    "mpr_latency_seconds",
    "High-res latency distribution trajectories",
    buckets=(0.005, 0.05, 0.5, 2.5),
)
CONCURRENT_JOBS = Gauge(
    "mpr_concurrent_executions", "Active runtime thread tracking allocations"
)
